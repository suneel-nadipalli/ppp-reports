...
async function loadJSON() {
  const res = await fetch("./final_report2.json");
  data = await res.json();
...
function dataMapFromLabel(label) {
  const [cpr, scn, mdl] = label.split(" | ");
  return data[cpr]?.[scn]?.[mdl];
}
...
modelBlocks.forEach(block => {
  ...
  const modelData = data[cpr.value]?.[scn.value]?.[mdl.value];
  const summary = modelData?.summary;

  if (summary) {
    const reportData = modelData.folds?.["1"]?.classification_report?.loss_report;

    selectedModels.push({
      label: `${cpr.value} | ${scn.value} | ${mdl.value}`,
      summary,
      report: (reportData && typeof reportData === "object") ? reportData : {}
    });
  }
});
...
function renderReport(models, opts) {
  ...
  // Show a second row for accuracy_report (if present)
const accuracyReportRow = document.createElement("div");
accuracyReportRow.className = "report-row";

models.forEach(m => {
  const accuracyData = dataMapFromLabel(m.label)?.folds?.["1"]?.classification_report?.accuracy_report;

  const wrapper = document.createElement("div");
  wrapper.className = "report-column";
  wrapper.innerHTML = `<h4>${m.label} (Accuracy Report)</h4>`;

  if (typeof accuracyData === "object") {
    const tbl = document.createElement("table");
    tbl.className = "report-table";
    tbl.innerHTML = `
      <thead><tr><th>Class</th><th>Precision</th><th>Recall</th><th>F1</th><th>Support</th></tr></thead>`;
    ...
    Object.entries(accuracyData).forEach(([cls, vals]) => {
      if (!vals || vals.precision === undefined) return;
      tb.innerHTML += `<tr><td>${cls}</td>
        <td>${vals.precision.toFixed(3)}</td>
        <td>${vals.recall.toFixed(3)}</td>
    ...
    wrapper.innerHTML += `<p>No accuracy report available.</p>`;
  }

  accuracyReportRow.appendChild(wrapper);
});

out.appendChild(accuracyReportRow);
